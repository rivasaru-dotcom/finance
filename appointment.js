...
async function renderTimes() {
  ...
  try {
    const response = await fetch(
      `${BOOKING_ENDPOINT}?action=availability&date=${encodeURIComponent(date)}`
    );
    ...
    availableTimes = result.available || [];
    ...
    BUSINESS_SLOTS.forEach(time => {
      ...
      els.timeGrid.appendChild(button);
    });
    ...
    els.timeGrid.innerHTML = `
      <div class="no-availability error">
        <i class="bi bi-exclamation-circle"></i>
        <strong>Couldn't check availability</strong>
